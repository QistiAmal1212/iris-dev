...
class DepartmentMinistryController extends Controller
{
            ...
            return Datatables::of($departmentMinistry)
                ...
                ->editColumn('action', function ($departmentMinistry) use ($accessUpdate, $accessDelete) {
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="departmentMinistryForm('.$departmentMinistry->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Kementerian";
            $log->data_new = json_encode($departmentMinistry);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
