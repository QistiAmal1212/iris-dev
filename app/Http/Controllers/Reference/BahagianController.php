...
class BahagianController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $bahagian = Bahagian::orderBy('kod', 'asc')->get();
        if ($request->ajax()) {
            ...
            return Datatables::of($bahagian)
                ...
                ->editColumn('nama', function ($bahagian) {
                    return $bahagian->diskripsi;
                })
                ->editColumn('action', function ($bahagian) use ($accessUpdate, $accessDelete) {
                    $button = "";
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="bahagianForm('.$bahagian->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="deleteItem('.$bahagian->id.')"> <i class="fas fa-trash text-danger"></i> ';
                    }
                    $button .= '</div>';
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Bahagian";
            $log->data_new = json_encode($bahagian);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
