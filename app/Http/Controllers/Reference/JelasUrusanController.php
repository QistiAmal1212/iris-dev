...
class JelasUrusanController extends Controller
{
            ...
            return Datatables::of($jelasurusan)
                ...
                ->editColumn('action', function ($jelasurusan) use ($accessUpdate, $accessDelete) {
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="jelasurusanForm('.$jelasurusan->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Jelas Urusan";
            $log->data_new = json_encode($jelasurusan);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
