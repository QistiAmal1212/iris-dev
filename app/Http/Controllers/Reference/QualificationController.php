...
class QualificationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($qualification->get())
                ...
                ->editColumn('action', function ($qualification) use ($accessUpdate, $accessDelete) {
                    $button = "";
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="qualificationForm('.$qualification->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }

                    if($accessDelete){
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="deleteItem('.$qualification->id.')"> <i class="fas fa-trash text-danger"></i> ';
                    }
                    $button .= '</div>';
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Kelulusan";
            $log->data_new = json_encode($qualification);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
