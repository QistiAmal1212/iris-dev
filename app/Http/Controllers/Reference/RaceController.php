...
class RaceController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($race)
                ...
                ->editColumn('nama', function ($race) {
                    return $race->diskripsi;
                })
                ->editColumn('action', function ($race) use ($accessUpdate, $accessDelete) {
                    $button = "";
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="raceForm('.$race->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }

                    if($accessDelete){
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="deleteItem('.$race->id.')"> <i class="fas fa-trash text-danger"></i> ';
                    }
                    $button .= '</div>';
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Keturunan";
            $log->data_new = json_encode($race);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
