...
class MaritalStatusController extends Controller
{
            ...
            return Datatables::of($maritalStatus)
                ...
                ->editColumn('action', function ($maritalStatus) use ($accessUpdate, $accessDelete) {
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="maritalStatusForm('.$maritalStatus->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Taraf Perkahwinan";
            $log->data_new = json_encode($maritalStatus);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
