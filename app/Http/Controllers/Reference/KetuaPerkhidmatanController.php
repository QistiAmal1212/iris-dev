...
class KetuaPerkhidmatanController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($ketuaperkhidmatan)
                ...
                ->editColumn('nama', function ($ketuaperkhidmatan) {
                    return $ketuaperkhidmatan->diskripsi;
                })
                ->editColumn('action', function ($ketuaperkhidmatan) use ($accessUpdate, $accessDelete) {
                    $button = "";
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="ketuaperkhidmatanForm('.$ketuaperkhidmatan->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="deleteItem('.$ketuaperkhidmatan->id.')"> <i class="fas fa-trash text-danger"></i> ';
                    }
                    $button .= '</div>';
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Ketua Perkhidmatan";
            $log->data_new = json_encode($ketuaperkhidmatan);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
