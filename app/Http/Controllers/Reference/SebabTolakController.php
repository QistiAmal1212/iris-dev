...
class SebabTolakController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($sebabtolak)
                ...
                ->editColumn('nama', function ($sebabtolak) {
                    return $sebabtolak->diskripsi;
                })
                ->editColumn('action', function ($sebabtolak) use ($accessUpdate, $accessDelete) {
                    $button = "";
                    ...
                    if($accessUpdate){
                        ...
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="sebabtolakForm('.$sebabtolak->id.')"> <i class="fas fa-eye text-primary"></i> ';
                    }
                    if($accessDelete){
                        $button .= '<a href="javascript:void(0);" class="btn btn-xs btn-default" onclick="deleteItem('.$sebabtolak->id.')"> <i class="fas fa-trash text-danger"></i> ';
                    }
                    $button .= '</div>';
    ...
    public function deleteItem(Request $request){
        ...
        try{
            ...
            $log->description = "Hapus Sebab Tolak";
            $log->data_new = json_encode($sebabtolak);
            $log->url = $request->fullUrl();
            $log->method = strtoupper($request->method());
            $log->ip_address = $request->ip();
