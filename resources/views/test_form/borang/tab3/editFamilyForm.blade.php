    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body px-sm-2 mx-50">
                ...
                <p class="text-center">{{$testTableFamily ? 'Edit' : 'Add'}} Family Member</p>

                <form id="familyForm" action="{{$testTableFamily ? route('testForm.updateFamily') : route('testForm.createFamily')}}" method="POST"  name="familyForm" data-refreshFunctionDivId="tableFamilyDiv" data-refreshFunctionURL="{{route('testForm.refreshFamilyTable')}}">
                        @csrf
                        <input type="hidden" name="test_table_id" value="{{$testTableFamily?->id ?? null}}">
                        <div class="row">
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    <label class="form-label" for="name">Name <span class="text text-danger">*</span> </label>
                                    <div class="input-group">
                                        <input type="text" id="name" name="name" class="form-control" placeholder="Full Name" required value="{{$testTableFamily?->name ?? ""}}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" id="age" name="age" class="form-control" placeholder="Age" required value="{{$testTableFamily?->age ?? ""}}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                ...
                                <select class="select2 form-select" id="select2-multiple" name="gender">
                                    ...
                                    <option value="Lelaki" {{$testTableFamily?->gender == "Lelaki" ? "selected" : ""}}>Lelaki</option>
                                    <option value="Perempuan" {{$testTableFamily?->gender == "Perempuan" ? "selected" : ""}}>Perempuan</option>
                                </select>
                            </div>
                        </div>
